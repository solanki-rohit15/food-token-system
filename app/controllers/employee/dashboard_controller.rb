class Employee::DashboardController < ApplicationController
  ...
  def index
    @today_orders  = current_user.orders.today.includes(:food_items, :tokens).order(created_at: :asc)
    @today_tokens  = @today_orders.flat_map(&:tokens)
    ...
    @recent_tokens = Token.for_user(current_user).includes(order_item: :food_item)
                          .order(created_at: :desc).limit(10)
    @ordered_categories = @today_orders.reject { |o| o.tokens.all?(&:expired?) }
